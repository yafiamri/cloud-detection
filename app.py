...
def detect_circle_roi(image_np, original_shape):
    ...
    return mask if coverage_ratio > 0.85 else np.ones(original_shape, dtype=np.uint8)
    ...
    if st.button("▶️ Proses"):
        for uploaded_file in uploaded_files:
            ...
            if roi_option == "Otomatis (Lingkaran)":
                ...
                st.warning(f"Silakan gambar ROI pada gambar: {filename}")
                    ...
                    if "Kotak" in roi_option:
                        ...
                        for obj in canvas_result.json_data["objects"]:
                            if obj["type"] == "path" and obj.get("path"):
                                coords = [
                                    [int(item[1]), int(item[2])] for item in obj["path"]
